...
- Method
  - Make spectra containing an absorption line with fixed parameters but with different continuum parameters and (Gaussian) noise levels.
  - For each spectrum, consider each case. ``Consider'' means find the optimal set of absorption line parameters using different objective functions in each case.
    - Simultaneous fit: objective function is sum of square standardized differences, get to vary line and continuum parameters
    - Marginalization for continuum: objective function is opposite of continuum-marginalized log-likelihood, get to vary line parameters
    - Known continuum: objective function is sum of square differences, only vary line parameters
    - Line-free portion guess: two objective functions, one for fitting continuum to line-free region, then use "known continuum" objective with result of first part to get absorption line part
  - To "guess" line-free region, motivated by an estimate of what a human looking through these spectra would do by eye.
    - Perfectly guess where the continuum passes by eye; perfectly guess what the optimal absorption line would look like given a perfect guess of where the continuum passes; pick the part of the spectrum where the optimal absorption line has negligible opacity.
    - Mechanically, use the part of the spectrum where the absorption line from the known continuum case has negligible opacity.
  - Consider continuum of order 0 (varying constant), 1 (line), 2 (quadratic).
  - Consider signal-to-noise levels  5TKTK to 25TKTK.
  - Do 1000TKTK continuum + noise realizations for each continuum order and SNR.
  - Define "well" as RMSE of log column density; includes bias and variance.
- Results:
  - Figure \ref{fig:order-known-comparison}
  - This is better than this in these regimes
- Discussion:
  - What does it mean that this is better than this?
  - How well should the result generalize to cases that were not tested?
